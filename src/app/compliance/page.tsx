...
function StatusBadge({ s }: { s: string }) {
  ...
  return <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full border ${cls[s]}`}>{sym[s]}</span>
}
  ...
  return (
    <div className="flex flex-col gap-1">
      <label className="text-xs font-medium text-slate-500">
        {label}{hint && <span className="ml-1.5 text-[10px] text-slate-400">{hint}</span>}
      </label>
...
export default function CompliancePage() {
  ...
  const set = (key: keyof ComplianceInputs, v: number) => setInputs(p => ({ ...p, [key]: v }))
    ...
    <div className="p-5 space-y-5">
      ...
      <div className="grid grid-cols-4 gap-3">
        {SCORES.map(s => (
          <div key={s.label} className="bg-white border border-gray-200 rounded-xl p-4">
            ...
            <div className="text-[28px] font-bold" style={{ color: s.score >= 90 ? '#10B981' : s.score >= 75 ? '#F59E0B' : PINK }}>{s.score}</div>
            <div className="w-full h-1.5 bg-gray-100 rounded-full mt-2 overflow-hidden">
              <div className="h-full rounded-full" style={{ width: `${s.score}%`, background: s.score >= 90 ? '#10B981' : s.score >= 75 ? '#F59E0B' : PINK }} />
            </div>
      ...
      <div className="flex gap-1 border-b border-gray-200">
        {TABS.map(t2 => (
          <button key={t2.id} onClick={() => setTab(t2.id)}
            ...
            style={{ borderBottomColor: tab === t2.id ? PINK : 'transparent', color: tab === t2.id ? PINK : '#6b7280' }}>
            {t2.label}
          </button>
        ))}
      </div>
      ...
      {tab === 'ratio' && (
        <div className="space-y-4">
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            <div className="text-xs font-semibold text-gray-700 mb-3">{t('residents')}: {inputs.totalResidents}</div>
            <div className="overflow-x-auto rounded-xl border border-gray-100">
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    {[t('col_role'), t('col_ft'), t('col_agency'), t('col_total'), t('col_min'), t('col_pt'), t('col_status')].map(h => (
                ...
                <tbody>
                  {r.checks.map(c => (
                    <tr key={c.label} className="border-b border-gray-50 hover:bg-gray-50/50">
                      <td className="px-3 py-2.5 font-semibold text-gray-700">{c.label}</td>
                      <td className="px-3 py-2.5 text-gray-600">{c.ft}</td>
                      <td className="px-3 py-2.5 text-gray-600">{c.agency}</td>
                      <td className="px-3 py-2.5 font-bold text-gray-800">{c.total}</td>
                      <td className="px-3 py-2.5 text-gray-600">{c.minReq}</td>
                      <td className="px-3 py-2.5 text-gray-500">{c.ptCap ?? '—'}</td>
                      <td className="px-3 py-2.5"><StatusBadge s={c.ok ? 'ok' : 'over'} /></td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {/* RCW PT ratio */}
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            <h2 className="text-sm font-semibold text-gray-900 mb-4">{t('rcw_title')}</h2>
            <div className="grid grid-cols-2 gap-3 mb-4 sm:grid-cols-4">
              <InputField label={t('rcw_ft')}   value={inputs.ftRcwCount}       onChange={v => set('ftRcwCount', v)}       suffix={t('suffix_person')} hint="March:19" />
              <InputField label={t('rcw_days')} value={inputs.ftRcwAvgWorkDays} onChange={v => set('ftRcwAvgWorkDays', v)} suffix={t('suffix_day')}    hint="avg:16" />
              <InputField label={t('rcw_pt')}   value={inputs.ptRcwShiftsMonth} onChange={v => set('ptRcwShiftsMonth', v)} suffix={t('suffix_shift')}  hint="March:111" />
              <div className="flex flex-col justify-center p-3 rounded-xl bg-gray-50 border border-gray-100">
                <div className="text-[10px] text-gray-500 mb-1">{t('rcw_max_pt')}</div>
                <div className="text-lg font-bold text-gray-800">{r.maxPtShifts}</div>
                <div className="text-[10px] text-gray-400">{r.ftShifts} × 50%</div>
              </div>
            </div>
            <div className="mb-1 flex justify-between text-xs text-slate-500">
              <span>{t('pt_usage')}</span>
              <span className={`font-bold tabular-nums ${r.ptStatus === 'over' ? 'text-red-600' : r.ptStatus === 'warn' ? 'text-amber-600' : 'text-emerald-600'}`}>{r.usagePct}% / 50%</span>
            </div>
            <div className="relative h-3 w-full rounded-full bg-gray-100 overflow-hidden mb-1">
              <div className="h-full rounded-full transition-all duration-700"
                style={{ width: `${Math.min(r.usagePct / 50 * 100, 100)}%`, background: r.ptStatus === 'over' ? '#ef4444' : r.ptStatus === 'warn' ? '#f97316' : '#10b981' }} />
            </div>
            <div className={`text-xs font-semibold mt-2 text-center rounded-lg py-1 ${r.remaining >= 0 ? 'bg-emerald-50 text-emerald-700' : 'bg-red-100 text-red-700'}`}>
              {r.remaining >= 0
                ? `${t('rcw_remaining')}: ${r.remaining} ${t('suffix_shift')}`
                : `${t('rcw_exceeded')} ${Math.abs(r.remaining)} ${t('suffix_shift')}`}
            </div>
          </div>
        </div>
      )}
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-5">
          <div className="overflow-x-auto rounded-xl border border-gray-100">
            <table className="w-full text-xs">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  {[t('cert_name'), t('cert_role'), t('cert_type'), t('cert_expiry'), t('cert_days'), t('cert_status')].map(h => (
                    <th key={h} className="px-3 py-2.5 text-[10px] font-semibold text-gray-500 uppercase tracking-wide text-left">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {CERTS.map(c => (
                  <tr key={c.name} className="border-b border-gray-50 hover:bg-gray-50/50">
                    ...
                    <td className="px-3 py-2.5 font-bold tabular-nums" style={{ color: c.daysLeft <= 7 ? '#ef4444' : c.daysLeft <= 30 ? '#f59e0b' : '#10b981' }}>{c.daysLeft}</td>
      ...
      {tab === 'agency' && (
        <div className="space-y-4">
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            <h2 className="text-sm font-semibold text-gray-900 mb-3">{t('violation_title')}</h2>
            <div className="overflow-x-auto rounded-xl border border-gray-100">
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    {[t('col_sev'), t('col_cat'), t('col_issue'), t('col_freq'), t('col_ref'), t('col_action')].map(h => (
                      <th key={h} className="px-3 py-2.5 text-[10px] font-semibold text-gray-500 uppercase tracking-wide text-left">{h}</th>
                    ))}
                  </tr>
                </thead>
                  ...
                  {VIOLATIONS.map((v, i) => (
                    <tr key={i} className="border-b border-gray-50 hover:bg-gray-50/50">
                      <td className="px-3 py-2.5">
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${v.sev === 'red' ? 'bg-red-100 text-red-700' : v.sev === 'amber' ? 'bg-amber-100 text-amber-700' : 'bg-yellow-100 text-yellow-700'}`}>
                          {v.sev === 'red' ? t('sev_high') : v.sev === 'amber' ? t('sev_mid') : t('sev_low')}
                        </span>
                      </td>
                      <td className="px-3 py-2.5 font-semibold text-gray-800">{v.category}</td>
                      <td className="px-3 py-2.5 text-gray-600">{v.issue}</td>
                      <td className="px-3 py-2.5 text-gray-500 whitespace-nowrap">{v.freq}</td>
                      <td className="px-3 py-2.5 text-[10px] text-gray-400 whitespace-nowrap">{v.ref}</td>
                      <td className="px-3 py-2.5 text-[10px] text-gray-600">{v.action}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {tab === 'audit' && (
        <div className="space-y-4">
          <div className="grid grid-cols-3 gap-3">
            {[
              { label: t('audit_ok'),   count: okCount,   color: '#10b981', bg: 'bg-emerald-50 border-emerald-200' },
              { label: t('audit_warn'), count: warnCount, color: '#f59e0b', bg: 'bg-amber-50 border-amber-200' },
              { label: t('audit_over'), count: overCount, color: '#ef4444', bg: 'bg-red-50 border-red-200' },
            ].map(s => (
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5">
            <div className="overflow-x-auto rounded-xl border border-gray-100">
              <table className="w-full text-xs">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-100">
                    {[t('col_cat'), t('col_issue'), t('col_ref'), t('col_freq'), t('col_status')].map(h => (
                      <th key={h} className="px-3 py-2.5 text-[10px] font-semibold text-gray-500 uppercase tracking-wide text-left">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {AUDIT_ITEMS.map((a, i) => (
                    <tr key={i} className="border-b border-gray-50 hover:bg-gray-50/50">
                      <td className="px-3 py-2.5 font-semibold text-gray-800">{a.item}</td>
                      <td className="px-3 py-2.5 text-gray-600">{a.detail}</td>
                      <td className="px-3 py-2.5 text-[10px] text-gray-400 whitespace-nowrap">{a.ref}</td>
                      <td className="px-3 py-2.5 text-gray-500">{a.freq}</td>
                      <td className="px-3 py-2.5"><StatusBadge s={a.status} /></td>
                    </tr>
                  ))}
                </tbody>
              </table>
