...
def test_staff_ai_analysis_is_evidence_backed(token):
    ...
    Read off a health worker, not the first RN. The Phase 4 task dictionary is
    profession-specific - daily codes belong to the care ranks - so a nurse whose
    month is all D shifts and leave has nothing to derive from, and reporting
    nothing for them is the correct answer rather than a bug.
    """
    ...
    # HW, because that is who we fetched. The rank assertion is a hard one - it
    # checks the endpoint returned the person we asked for. The evidence counts
    # are not: they depend on which of the two DB fixtures is loaded, and a demo
    # seed and an imported real roster carry different amounts.
    assert body["staff"]["rank"] == "HW"
    require_count(body["activity"]["working_shifts"], "rostered shifts for an HW")
