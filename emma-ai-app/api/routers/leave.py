...
def create_request(body: LeaveRequestCreate, ctx: AuthCtx = Depends(get_ctx)):
    ...
    return svc.create_request(
        ctx.client, ctx.facility_id, staff_id=staff_id, leave_type=body.leave_type,
...
def decide_request(request_id: str, body: LeaveDecisionRequest,
    ...
    return svc.decide(ctx.client, ctx.facility_id, request_id,
                      decision=body.decision, profile_id=ctx.profile_id, note=body.note)
